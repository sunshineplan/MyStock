...
class SSE:
    def __init__(self, code=None):
        ...
        try:
            response = requests.get(
                f'http://yunhq.sse.com.cn:32041/v1/sh1/snap/{self.code}', timeout=1)
            if response.status_code == 200:
                self.snap = response.json()
                self.exist = True
            else:
                self.exist = False
        except:
    ...
    def chart(self):
        if self.exist:
            try:
                response = requests.get(
                    f'http://yunhq.sse.com.cn:32041/v1/sh1/line/{self.code}', timeout=1)
...
class SZSE:
    def __init__(self, code=None):
        ...
        try:
            response = requests.get('http://www.szse.cn/api/market/ssjjhq/getTimeData',
                                    params={'marketId': 1, 'code': self.code}, timeout=1).json()
